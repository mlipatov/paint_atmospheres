...
ind_g = np.searchsorted(ld.g_arr, logg)
print('Gravity removed from intensity information: ' + str(ld.g_arr[ind_g]) + \
	'. \nThese should be the same as the star gravity.')
print('Gravities between which we are interpolating: ' + str(ld.g_arr[ind_g - 1]) +\
	' and ' + str(ld.g_arr[ind_g + 1]) )
...
for i in range(3):
	...
	axarr[i].scatter(wl_arr, diff_log[i], marker='o', c='g', s=6, alpha=1, edgecolors="none")
